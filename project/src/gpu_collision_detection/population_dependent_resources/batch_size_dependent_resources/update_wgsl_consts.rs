...
use crate::{
    gpu_collision_detection::resources::{MaxDetectableCollisionsScale, WorkgroupSizes},
...
pub fn update_wgsl_consts(
    ...
    gpu_workgroup_sizes: Res<WorkgroupSizes>,
    ...
    if batch_population.0 > 0 {
        if batch_population.is_changed()
            ...
            || gpu_workgroup_sizes.is_changed()
        {
            ...
            num_gpu_workgroups_required.0.0 =
                (batch_population.0 as f32 / gpu_workgroup_sizes.0.0 as f32).ceil() as usize;
            num_gpu_workgroups_required.0.1 =
                (batch_population.0 as f32 / gpu_workgroup_sizes.0.1 as f32).ceil() as usize;
            num_gpu_workgroups_required.0.2 =
                (batch_population.0 as f32 / gpu_workgroup_sizes.0.2 as f32).ceil() as usize;
        }
    }
}
